...
fn sum_over_non_adjacent_dims() {
    ...
    assert_eq!(*out.at(&[0]), 24.0);
    assert_eq!(*out.at(&[1]), 42.0);
}
...
fn sum_over_adjacent_dims() {
    ...
    assert_eq!(out.shape(), &[2]);
    assert_eq!(*out.at(&[0]), 15.0);
    assert_eq!(*out.at(&[1]), 51.0);
}
...
fn sum_single_dim_uses_single_kernel() {
    ...
    assert_eq!(*a.at(&[0, 1]), 5.0);
    assert_eq!(*a.at(&[0, 2]), 7.0);
    assert_eq!(*a.at(&[1, 0]), 15.0);
    assert_eq!(*a.at(&[1, 1]), 17.0);
    assert_eq!(*a.at(&[1, 2]), 19.0);
    ...
    assert_eq!(*one_shot.at(&[]), *chained.at(&[]));
    assert_eq!(*one_shot.at(&[]), 66.0);
}
...
fn sum_keepdim_keeps_reduced_axes() {
    ...
    assert_eq!(*out.at(&[0, 0, 0]), 24.0);
    assert_eq!(*out.at(&[0, 1, 0]), 42.0);
}
...
fn mean_keepdim_single_dim() {
    ...
    assert_eq!(*out.at(&[0, 0, 1]), 2.5);
    assert_eq!(*out.at(&[1, 0, 2]), 9.5);
}
...
fn max_keepdim_keeps_reduced_axes() {
    ...
    assert_eq!(out.shape(), &[1, 2, 1]);
    assert_eq!(*out.at(&[0, 0, 0]), 8.0);
    assert_eq!(*out.at(&[0, 1, 0]), 11.0);
}
