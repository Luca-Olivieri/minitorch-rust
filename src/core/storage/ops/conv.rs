...
pub(crate) fn conv2d_backward<T: Numeric>(
    ...
    let mut packed_grad_weight = vec![T::ZERO; in_channels * kernel_h * kernel_w * out_channels];
                ...
                for ic in 0..in_channels {
                    ...
                    for kh in 0..kernel_h {
                        ...
                        for kw in 0..kernel_w {
                            ...
                            let packed_base = ((ic * kernel_h + kh) * kernel_w + kw) * out_channels;
                            let grad_weight_slice =
                                &mut packed_grad_weight[packed_base..packed_base + out_channels];
